...
public class OrderService {
    ...
    public OrderDTO saveOrder(String orderDTOStr, MultipartFile file) {
        try {
            ...
            Order order = makeOrderFromOrderDTO(orderDTO, new Order());

            // Сначала сохраняем Order, чтобы получить ID
            Order savedOrder = orderRepository.save(order);

            // Обновляем DTO с сохраненным Order ID
            orderDTO.setId(savedOrder.getId());

            // Сохранение файла, если он присутствует
            if (file != null) {
            ...
            // Пересчитываем orderDTO после сохранения
